...
format_corr <- function(x,
  ...
  # Build label
  r_lab <- dplyr::case_when(!italics ~ paste0("r"),
                            identical(type, "md") ~ paste0("_r_"),
                            identical(type, "latex") ~ paste0("$r$"),
  )
  rlab <- dplyr::case_when(identical(dfs, "par") ~ paste0(r_lab, "(", df, ")"),
                           identical(dfs, "sub") & identical(type, "md") ~ paste0(r_lab, "~", df, "~"),
                           identical(dfs, "sub") & identical(type, "latex") ~ paste0(r_lab, "$_{", df, "}$"),
                           identical(dfs, "none") ~ r_lab)

  # Create statistics string
  full_lab <- dplyr::case_when(ci ~ paste0(rlab, " = ", corr, ", 95% CI [", cis[1], ", ", cis[2], "], ", pvalue),
                               !ci ~ paste0(rlab, " = ", corr, ", ", pvalue))
  return(full_lab)
}
